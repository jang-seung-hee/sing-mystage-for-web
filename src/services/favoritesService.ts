...
export async function addFavorite(video: YouTubeSearchResultItem, folderId?: string) {
  ...
  const processKey = `${user.uid}_${videoId}_${folderId || 'default'}`;
  ...
  try {
    ...
    // 같은 폴더에 같은 곡이 이미 있으면 추가하지 않음
    const q = query(collection(db, FAVORITES_COLLECTION), where('uid', '==', user.uid));
    const snapshot = await getDocs(q);
    const alreadyExists = snapshot.docs.some((docSnap) => {
      const data = docSnap.data();
      const existingVideoId =
        typeof data.video?.id === 'string' ? data.video.id : data.video?.id?.videoId;
      const existingFolderId = data.folderId || null;
      return existingVideoId === videoId && (existingFolderId === (folderId || null));
    });
    if (alreadyExists) {
      console.log(`addFavorite: 이미 같은 폴더에 존재합니다. videoId: ${videoId}, folderId: ${folderId}`);
      return;
    }
...
export async function moveFavoritesToDefaultFolder(deletedFolderId: string) {
  ...
  const movePromises = snapshot.docs
    ...
    .map((doc) => updateDoc(doc.ref, { folderId: null }));
...
export async function getFolderBundle(folderId?: string): Promise<{
  ...
  if (folderId) {
    ...
    const folderQuery = query(collection(db, 'favorite_folders'), where('uid', '==', user.uid));
