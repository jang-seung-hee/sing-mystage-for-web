...
import { SharedFolder } from './sharedFavoritesService';
...
export async function getFolders(): Promise<FolderItem[]> {
  ...
  try {
    ...
    const folders = sortedDocs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    } as FolderItem));
...
export async function importOnlineFolder(sharedFolder: SharedFolder): Promise<string> {
  ...
  try {
    ...
    console.log(`온라인 폴더 가져오기 완료: ${sharedFolder.title} (${successCount}/${sharedFolder.favorites.length}개 항목)`);
